...
class Board:
        ...
        for i in range(self.size):
            for j in range(self.size):
                if isinstance(self.rows[i][j], list):
                    if len(self.rows[i][j]) < shortest_len:
                        shortest_len = len(self.rows[i][j])
                        cell = (i,j)
        return cell
    ...
    def update(self, row: int, column: int, assignment: int) -> None:
        ...
        for t in lst:
            remove_if_exists(self.rows[t[0]][t[1]], assignment)
...
def DFS(state: Board) -> Board:
    ...
    mystack = Stack()
    myboard = Board()
    mystack.push(myboard)

    while not mystack.is_empty():
        curr = mystack.the_stack[-1]
        if curr.goal_test() == True:
            return curr
        elif curr.failure_test() == False:
            (r,c) = curr.find_most_constrained_cell()
            for i in curr.rows[r][c]:
                myboard_copy = copy.deepcopy(curr)
                myboard_copy.update(r, c, i)
                mystack.push(myboard_copy)
    return None
...
def BFS(state: Board) -> Board:
    ...
    myqueue = Queue()
    myboard = Board()
    myqueue.push(myboard)

    while not myqueue.is_empty():
        curr = myqueue.the_queue[-1]
        if curr.goal_test() == True:
            return curr
        elif curr.failure_test() == False:
            (r,c) = curr.find_most_constrained_cell()
